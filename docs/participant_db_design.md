...
第1段階ではDBの受け皿として `org1` から `org5` までを持ちます。
ただし通常入力画面と通常CSVでは、当面 `org1` と `org2` の2項目だけを前面に出します。

例:

- `org1` = 今までの `organization` 相当。通常表示に使う所属名。
- `org2` = 短縮・表示調整用の所属名。
- `org3` = 将来拡張用。
- `org4` = 将来拡張用。
- `org5` = 将来拡張用。

表示側では、まず `org1` を既存の所属表示として扱います。
`org2` は、SVGや採点票PDFで短縮表示・表示調整が必要になったときの候補として保持します。
`org3` 以降は通常画面には出さず、全国大会などで複数所属表示が必要になった段階で開放します。
将来的には、どの所属項目を使うか、どの順で出すか、選手名の横に出すか下に出すかを大会設定で決められるようにします。
...
- 所属はペア共通入力を標準とし、CSVの第1段階正式ヘッダは `org1` と `org2` とする。
...
- 旧 `organization` は読み取り互換として受け付け、`org1` として扱う。
...
- 通常入力画面と標準CSVフォーマットで前面に出す所属項目は、当面 `PARTICIPANT_ORGANIZATION_INPUT_CODES` の `org1` / `org2` に限定する。

第1段階の運用整理:

- DB上は player1 / player2 それぞれの所属値を持てる構造にする。
- ただし通常運用・通常画面では、当面は player2 側の所属を個別入力・個別表示の対象にしない。
- 参加者作成・編集画面では、所属入力欄はペア共通の `org1` と `org2` を基本にする。
- ペア共通の所属値は、保存時に player1 / player2 の両方へ同じ値として保存する。
- 表示上は、当面 player1 側の `org1` と `org2` をペアの代表所属として扱う。
...
- 旧 `organization` はCSV取込では読めるが、第1段階の通常CSVテンプレートには出さない。
...
- CSVの第1段階正式ヘッダは `org1` と `org2` とする。
...
- 参加者作成・編集画面では、通常入力として `org1` と `org2` を入力できるようにする。
- `org3` から `org5` はDBとCSV読み取り互換の受け皿として残し、通常画面には前面に出さない。
...
将来、Bootstrap、HTMX、または別のUIライブラリへ寄せる可能性があるため、参加者管理やステージ作成画面のUIは、特定の見た目実装に依存しすぎないようにします。

基本方針:

- UI上の意味や操作対象は、できるだけ `data-*` 属性に持たせる。
- CSSクラスは見た目と状態表示を担当し、業務ロジックをCSSクラス名に強く依存させない。
- JavaScriptは「クリックされたボタン専用の処理」ではなく、「受け取ったデータを対象へ反映する処理」を小さな関数にまとめる。
- クリック、タップ、ドラッグ&ドロップは、可能な限り同じ反映関数を使う。
- ドラッグ&ドロップを実装しても、タッチ端末や操作に慣れていない利用者のためにクリック/タップ操作は必ず残す。
- 保存処理やDB更新処理は `core.services` に寄せ、テンプレートやJavaScriptから直接DB更新の意味を持たせない。
- 見た目の部品は、初期段階では画面専用クラスで始めてもよいが、安定したら `form-panel`、`choice-button`、`drop-zone` のような共通部品へ寄せる。
- 外部UIライブラリへ移行するときは、`data-*` とサービス層の責務を維持し、見た目クラスだけ置き換えられる構造を目指す。

参加者フォームの所属セット候補では、この方針の第1段階として、クリックとドラッグ&ドロップの両方が同じ所属反映処理を通るようにします。
将来、参加者カードをリーグ枠・トーナメント枠・進行枠へ貼り付ける画面でも、同じ考え方を使います。

## 参加者編集時の影響確認

参加者情報は、リーグ表、トーナメント表、進行表、採点票PDF、公開表示で参照されます。
ただし、結果入力済みの試合に使われている参加者であっても、実運用上は「試合開始前から本来変更済みだったが、システムへの反映が遅れた」「誤登録を後から直した」だけの場合があります。
そのため、編集タイミングを根拠に結果表示や画面表示を過去状態へ戻す制御は行いません。

基本方針:

- 表示は常に現在の参加者情報を使う。
- 編集タイミングによって、過去の結果表示を別扱いしない。
- 結果入力済みの参加者でも、通常編集は保存可能にする。
- ただし、既にリーグ枠、トーナメント枠、進行表、結果入力済み試合で使われている場合は、保存前に一度だけ確認を挟む。
- 確認はブラウザの `confirm()` だけに頼らず、サーバ側で「確認済み」のPOST値を見て二段階保存にする。
- 確認後に保存しても、試合結果そのものは変更しない。表示名・所属など、参加者情報を参照する表示だけが更新後の内容になる。
- 将来の変更履歴では、いつ誰が何を変更したかを証跡として残す。ただし履歴は通常表示の出し分けには使わない。

第1段階では、参加者編集画面で影響件数を表示し、「影響を確認して保存」を押した場合だけ保存します。
この確認は誤操作防止のための注意喚起であり、編集を禁止するものではありません。
