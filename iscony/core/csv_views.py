...
CSV_FORMAT_FILENAMES = {
    ...
    "league_entries": "league_entries_format.csv",
...
def import_pairs(request, tournament_code):
    """
    リーグ枠CSVを取り込む。

    LeagueEntry はリーグ表の各コマに入るリーグ枠として扱う。
    取込対象カテゴリの Group, LeagueEntry, RoundRobinMatch, Schedule,
    GroupRanking を作り直し、最後にリーグ内総当たりを生成する。
    """
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            # CSVに含まれるカテゴリだけ既存データの置換対象にする。
            category_names = set()

            # 先に全行を検証して、途中までDB更新された状態を避ける。
            for row in rows:
                category_names.add(
                    row["category"].strip()
                )
            ...
            for category_name in category_names:
                ...
                Schedule.objects.filter(
                    round_robin_match__group__in=groups
                ).delete()
            ...
            # 後で総当たりを作るため、今回作成・更新したGroupを覚える。
            created_groups = set()
            ...
            # リーグ枠が揃ったら、同じグループ内の全対戦を生成する。
            groups = Group.objects.filter(
                id__in=created_groups
            )
